...
function evaluateTicTacToePosition(position) {
  ...
  const linesToCheck = [...lines, ...diagonals];
  const winner = linesToCheck.find((line) => {
    const [first, second, third] = line;
    return first === second && second === third && first !== undefined;
  });
  if (winner) {
    return winner[0];
  }
  const emptyCells = position.reduce((acc, row) => acc
    .concat(row.filter((cell) => cell === undefined)), []);
